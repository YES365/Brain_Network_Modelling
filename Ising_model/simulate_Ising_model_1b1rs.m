function [spins] = simulate_Ising_model_1b1rs(J, h, T, steps)
%SIMULATE_ISING_MODEL_1B1R 模拟不均匀网络上的Ising模型，每个 MC 步随机遍历N个节点
% 输入参数:
...
for step = 1:init_steps
    randomSequence = randperm(N);
    for trial = 1:N % 每个 MC 步包含 N 次尝试
        % 随机选择一个自旋
        i = randomSequence(trial);
